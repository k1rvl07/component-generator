...
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand("extension.createComponent", (uri) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield checkAndUpdatePackageJson(workspaceRoot);
        }
        catch (error) {
        ...
        try {
            yield generateComponents(data, folderPath, workspaceRoot);
...
function checkAndUpdatePackageJson(workspaceRoot) {
    return __awaiter(this, void 0, void 0, function* () {
        const packageJsonPath = path.join(workspaceRoot, "package.json");
        if (!fs.existsSync(packageJsonPath)) {
            const createPackageJson = yield vscode.window.showInformationMessage("package.json not found. Do you want to create it with 'type': 'module'?", "Yes", "No");
            if (createPackageJson === "Yes") {
                const packageJsonContent = JSON.stringify({ type: "module" }, null, 2);
                yield writeFile(packageJsonPath, packageJsonContent, "utf8");
            ...
            if (!packageJson.type || packageJson.type !== "module") {
                const updatePackageJson = yield vscode.window.showInformationMessage("package.json does not have 'type': 'module'. Do you want to add it?", "Yes", "No");
                if (updatePackageJson === "Yes") {
                    packageJson.type = "module";
                    yield writeFile(packageJsonPath, JSON.stringify(packageJson, null, 2), "utf8");
...
function generateComponents(componentsData, folderPath, workspaceRoot) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const data of componentsData) {
            ...
            if (fs.existsSync(componentPath)) {
                const replaceComponent = yield vscode.window.showInformationMessage(`A component with the name "${data.name}" already exists. Do you want to replace it?`, "Yes", "No");
            ...
            if (isBiome) {
                ...
                yield new Promise((resolve) => setTimeout(resolve, 2000)); // Задержка 2 секунды
                if (fs.existsSync(componentPath)) {
